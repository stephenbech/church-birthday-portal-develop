...
//   const churchesArray= userContext.churchesTable.map(({Id, Name, GroupId })=> {
...
//   } )
  ...
  function sendEmail(e?: { preventDefault: () => void; }) {
      ...
      params.set('phone', `${phoneRef.current?.value}`)
              ...
              <body>
                   <div class="emailContainer" style = "background-color: #f9f9f9; margin: 0px auto;">
                        ...
                        <div  class="content">
                               ...
                               <p class="sign-up">Click <span><a href="http://localhost:3001/create-account?${params.toString()}" id="signUpLink" >here</a></span> to proceed to Create your Account</p>
                        </div>
                   </div>
              </body>
           </html>`
      ...
      setContent(
        ...
        <form>
          ...
          <div className='input__wrapper'>
            ...
            <select className='finput' placeholder='Select Role' id='role' defaultValue={updateRole} onChange = {event => setUpdateRole(event.target.value)}>
                  ...
                  <option  className='finput' value="Admin">Admin</option>
...
// Modals End Here
  ...
  function updateAdmin(e?: { preventDefault: () => void; id: string}){
    let role = document.getElementById('role') as HTMLSelectElement;
    let church = document.getElementById('church') as HTMLSelectElement;
    const admin = {
  ...
  function handleUpdateAdmin(this: any) {
      ...
      setUpdateGender(tableRow.cells[3].innerHTML);
      setUpdateEmail(tableRow.cells[4].innerHTML);
      setUpdatePhone(tableRow.cells[5].innerHTML);
      setUpdateRole(tableRow.cells[6].innerHTML);
      setUpdateChurch(tableRow.cells[7].innerHTML);
  ...
  function handleViewAdmin(this: any) {
    ...
    setUpdateRole(tableRow.cells[6].innerHTML);
    setUpdateChurch(tableRow.cells[7].innerHTML);
    setUpdateGroup(tableRow.cells[8].innerHTML);
  ...
  const headers = ['ID','First Name', 'Last Name', 'Gender', 'Email', 'PhoneNumber', 'Role', 'Church', "Actions "]
  const adminArray = userContext.adminsTable.map(({Id, FirstName, LastName, Gender, Email, PhoneNumber, Role, Church}) => {
      return {Id, FirstName, LastName, Gender, Email, PhoneNumber, Role, Church}
  })
  const actions = <React.Fragment >
      ...
      <button className='table__button table__view__button' onClick={() => { deleteMember(id);}}>
